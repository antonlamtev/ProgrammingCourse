...
\item \textbf{Логика}
\begin{itemize}
\item \textbf{\textit{check\_for\_beating()}}
...
\begin{itemize}
\item \textbf{\textit{queens\_result()}}

Эта функция определяет, какой ферзь, кого бьет. Имеет три параметра (3 ферзя) структурного типа, объединяющего два целочисленных поля - две координаты ферзя. Далее она несколько раз вызывает функцию check\_for\_beating() и для каждой пары ферзей вычисляет резултат. Возвращаемое значение имеет тип \textbf{\textit{int}} -- один элемент из перечисления \textit{\textbf{enum}}, название которого характеризует результат.
\end{itemize}

\textbf{\item Взаимодействие с пользователем}

\begin{itemize}
\item \textbf{\textit{input\_with\_check()}}
...
\end{itemize}
\begin{itemize}
\item \textbf{\textit{display\_result()}}
